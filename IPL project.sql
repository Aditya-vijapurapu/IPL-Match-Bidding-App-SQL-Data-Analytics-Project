...
SELECT 
    b.BIDDER_ID,
    ROUND(
        (SUM(CASE WHEN b.BID_STATUS = 'Won' THEN 1 ELSE 0 END) * 100.0) 
...
SELECT 
	...
	/ COUNT(m.match_id),2) AS toss_win_match_win_percentage
FROM ipl_match_schedule as ms
JOIN ipl_stadium as s    ON ms.stadium_id = s.stadium_id
JOIN ipl_match as m      ON ms.match_id = m.match_id
...
SELECT 
    bd.bid_team AS team_id,
    t.team_name,
    COUNT(*) AS total_bids
FROM ipl_bidding_details as bd
JOIN ipl_team as t    ON t.team_id = bd.bid_team
...
SELECT it.Team_ID, it.Team_Name, 
	sum(matches_played) as Total_Match_Played, 
	sum(matches_won) Total_Match_Won, 
    sum(matches_lost) Total_Match_Lost
FROM ipl_team_standings s 
JOIN ipl_team it        ON s.Team_ID = it.Team_ID
...
from ipl_player 
where player_id in(
	select player_id from ipl_team_players where team_id in(
...
select team_id,
	   team_name,
	   count(*)as no_of_allrounders
from ipl_team_players p
join ipl_team as t using(team_id)
...
SELECT  Bid_status, 
		YEAR(Bid_date) as Bidding_Year, 
		sum(total_points) as Total_Bidder_Points
FROM ipl_bidding_details as bd
JOIN ipl_bidder_points as bp USING (bidder_id)
JOIN ipl_match_schedule as ms USING (schedule_id)
JOIN ipl_match as m USING (Match_id)
WHERE ms.stadium_id = (
	SELECT Stadium_id 
    from ipl_stadium as s 
    WHERE m.win_details like '%CSK%' AND s.Stadium_name = 'M. Chinnaswamy Stadium')
...
(SELECT player_id, 
dense_rank() over(order by substring(performance_dtls, instr(performance_dtls,"Wkt") +4,(instr(performance_dtls,"Dot") -5)-instr(Performance_dtls,"Wkt"))desc) AS ranking
FROM ipl_player 
...
(SUM(CASE WHEN (m.TEAM_ID1 = bg.BID_TEAM AND m.TOSS_WINNER = 1) OR
                  (m.TEAM_ID2 = bg.BID_TEAM AND m.TOSS_WINNER = 2) THEN 1 ELSE 0 END) / COUNT(*)) * 100 AS Toss_Win_Percentage
FROM ipl_match m
INNER JOIN ipl_match_schedule schd ON m.MATCH_ID = schd.MATCH_ID
INNER JOIN ipl_bidding_details bg ON schd.SCHEDULE_ID = bg.SCHEDULE_ID
INNER JOIN ipl_bidder_details bd ON bg.BIDDER_ID = bd.BIDDER_ID
INNER JOIN ipl_bidder_points pts ON bd.BIDDER_ID = pts.BIDDER_ID
...
INNER JOIN ipl_bidder_points as pts ON bd.bidder_id = pts.bidder_id
INNER JOIN ipl_bidding_details as bg ON pts.bidder_id = bg.bidder_id
...
INNER JOIN ipl_bidder_details as bd ON bg.bidder_id = bd.bidder_id
INNER JOIN ipl_bidder_points as pts ON bg.bidder_id = pts.bidder_id
WHERE year(bg.bid_date) = 2017
...
SELECT
    ...
    rb.total_points,
    CASE
        WHEN rb.high_rank <= 3 THEN 'Highest_3_Bidders'
        WHEN rb.low_rank  <= 3 THEN 'Lowest_3_Bidders'
    END AS bidder_category
FROM ranked_bidders rb
JOIN ipl_bidder b
    ON b.bidder_id = rb.bidder_id
WHERE rb.high_rank <= 3
   OR rb.low_rank  <= 3
ORDER BY rb.total_points DESC;